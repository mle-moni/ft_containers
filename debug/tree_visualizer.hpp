#ifndef TREE_VISUALISER_H
#define TREE_VISUALISER_H
...
template <class Key>
void showTree(const tree_node<Key> *root, std::string file_name = std::string("tree.png"))
{
	if (root == nullptr)
	...
	unordered_map<const tree_node<Key> *, int> hash;
	queue<const tree_node<Key> *> q;
	q.push(root);
	for (int idx = 0, nullidx = 0; !q.empty(); )
	{
		const tree_node<Key> *p = q.front();
		q.pop();
		if (p == nullptr)
	...
	// 调用GraphViz
	std::string command = "dot tree.dot | gvpr -c -f binarytree.gvpr | neato -n -Tpng -o ";
	command += file_name;
	int ret = 0;
